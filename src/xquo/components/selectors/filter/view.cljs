...
(defn filter
  ...
  [{:keys [selected? size background on-press-in on-press-out icon]
    :or   {size       32
           background :none}
    :as   props}
   content]
     ...
     [:rn/pressable (-> props
                        ...
                        (assoc :hit-slop     6
                               ...
                               :style        (rec.xf/add-styles
                                              ...
                                              (:style props))))
      (if (= size 24)
        [icon/view (merge {:name  :icon/placeholder
                           :size  12
                           :color icon-color}
                          icon)]
        [icon/view (merge {:name  :icon/unread
                           :size  20
                           :color icon-color}
                          icon)])
      (when text?
